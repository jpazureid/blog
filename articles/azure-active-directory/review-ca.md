...
条件付きアクセスを一言でいうと、Microsoft 365 のサービスや Salesforce など、クラウド上で提供されているサービスへのアクセスを制御する機能です。

条件付きアクセスは (不正な) サインインをブロックする機能であり、許可する機能がありません。つまり、「ブロックする」 or 「ブロックの対象外にする (結果として許可される)」 という制御の選択と想定ください。
一例をあげると、「多要素認証を要求する」という制御は、「多要素認証の要求に応答できれば許可」ではなく、「多要素認証の要求に応えられなければサインインをブロック」という制御です。
