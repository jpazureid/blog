...
- マネージド ID は個別の資格情報 (クライアント シークレット文字列 or 証明書) を持たない

マネージド ID では、Azure 内部ののエンドポイントが各リソースのシステム ID を認証し、Microsoft Entra ID での認証処理を行います。</br>
資格情報が存在しないため、資格情報の管理 (有効期限に応じた更新、資格情報漏洩の対策 など) が不要というメリットがあります。
...
上述のとおり、マネージド ID では資格情報の管理が不要という大きなメリットがあります。そして、管理者側で予めマネージド ID を有効化した Azure リソース上でのみ、マネージド ID 認証が利用可能であるため、外部のシステムによる不正利用ができないというセキュリティ面での利点もあります。
<br>


マネージド ID が利用できないシナリオでは、次点で、サービス プリンシパルの証明書認証がおすすめです。<br>

証明書認証も利用が難しい場合には、サービス プリンシパルのクライアント シークレット認証をご検討ください。クライアント シークレット認証では、資格情報の平文がスクリプト内に記載されるため、取り扱いには十分ご注意ください。<br>
必要に応じて、後述の「ワークロード ID 用の条件付きアクセス」もぜひご活用ください。
...
サービス プリンシパル認証 (シークレット・証明書) による認証のセキュリティ性をより高めたい場合は、ワークロード ID 用の条件付きアクセスの活用をおすすめします。
...
<活用例><br>
サービス プリンシパル認証を利用する IP アドレス範囲が予め分かっている場合に、その他の IP アドレスからのアクセスをブロックとするポリシーを作成します。これにより、不正アクセス発生時のリスクを低減いただけます。


## サービスプリンシパル認証・マネージド ID 認証に対応した PowerShell モジュール (一部)
