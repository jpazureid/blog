...
従来、条件付きアクセスは "ユーザー" の "リソース" へのサインインを制御する機能でした。今回のプレビューによって、"ワークロード ID" というアプリケーション(サービスプリンシパル) のサインインに対して IP アドレス ベースの制御を構成することが可能になりました。様々な機能が追加されてきている条件付きアクセスにおいて、遂に！サービスプリンシパルのサインインに対しても制御することができるようになったのかと感慨深い気持ちになったため紹介します。
...
環境内のワークロード ID を保護することは、悪意のある人物がこれらの ID を侵害し、使用して不正にアクセスすることを防ぐために重要です。
...
動作としては思った通りの結果を得て、構成できる制御も少ないですが、個人的には遂にサービス プリンシパルのサインインに対して制御を構成できるようになったことにとっても感心していました。Azure AD には様々な機能が追加されているため、今後の展開が楽しみです。
