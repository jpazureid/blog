...
> [!NOTE]
> 2022 年 6 月 13 日更新: 本ブログの内容を更新いたしました。一度ご確認済みの方も改めて本記事をご確認ください。変更点の概要を確認されたい場合は、「ポップアップが表示されるテナントの場合」 の項目をご確認ください。
...
## この記事の要点は ?
**2019 年 10 月 22 日より前に作成された一部の Azure AD テナントにおいて、「セキュリティの既定値群」の有効化を促すポップアップが表示されるようになります。実施時期の目安は 2022 年 6 月末頃を予定しています。<span style="color: red; ">ポップアップが表示されたテナントでは 14 日経過したタイミングで 「セキュリティの既定値群」 が自動的に有効化される場合があります。自動的に有効化されることを避ける必要がある場合、 「セキュリティの既定値群」を一度有効化し、有効化後に再度無効化してください (対処方法を後述しています)。</span>**
...
4. 上記プロンプトで "Ask Later" を選択いただくと、~~14 日後に再度、同様のポップアップが表示されます。その際には、セキュリティの規定値群を引き続き "無効" のままご利用いただくオプションも、選択肢として表示される見込みです。~~ 2 日間上記通知は表示されなくなりますが、2 日後に再度表示されます (再度 Ask later が可能)。合計 14 日間 Ask later で先延ばしが可能です。

### ポップアップが表示された場合どうなるか
- 後述する 「再無効化」 の対処をしない場合、14 日経過したタイミングで 「セキュリティの既定値群」 が自動的に有効化される場合があります。(自動的に有効化される条件は前述の「影響を受ける可能性があるテナント」 以上の情報は公開されておりません)。
-  「セキュリティの既定値群」 の無効化状態を維持する必要がある場合、 「セキュリティの既定値群」を一度有効化し、有効化後に再度無効化することで、「セキュリティの既定値群」の有効化を促すポップアップは表示されなくなり、また、セキュリティの既定群の自動的な有効化も避けることができます。(この手法を本ブログでは **「再無効化」** と呼称しています)
  - 再無効化において、有効化から無効化までは特に間隔を空ける必要はありません。
  - 再無効化はポップアップが表示される前に実施しても効果があると考えれらますが、タイミング次第では再無効化を実施してもポップアップが表示される可能性があります。この場合は、ポップアップが表示されたのちに再度、再無効化を実施ください。
...
## 補足 2 : 上記変更に至った経緯
そもそもなぜ、セキュリティの既定値群の有効化推奨のポップアップ表示に至ったのか？その経緯と背景については、冒頭に記載の「Raising the Baseline Security for all Organizations in the World」にてご紹介しております。以下はその抄訳です。
...
しかし 2019 年 10 月より前に作成された Azure AD テナントでは、「セキュリティの規定値群」は有効化されておりません。そのため、別途条件付きアクセス ポリシー等の他の機能によってセキュリティ性を高めていただいている場合を除き、前述した「パスワードのみの認証」によるリスクに晒されている現状を懸念して、今回の「セキュリティの規定値群の有効化推奨のポップアップ表示」のロールアウトを決定いたしました。
...
***
Q2. 事前通知はグローバル管理者へのメール以外にもなされますか。

A2. はい、メールだけでなく、M365 Message Center と Azure ポータル上での表示が行われる見込みです。
***
Q3. CSP などパートナーのテナントでも表示されますか？
