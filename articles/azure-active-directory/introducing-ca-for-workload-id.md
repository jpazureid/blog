...
マネージド ID はアプリケーションに資格情報を埋め込むことなく安全にリソースアクセスが可能な方法であり、制御の対象になりません。
...
次のコマンドを順に実行します。
グローバル管理者でのサインイン時に、Microsoft Graph PowerShell SDK に対する Application.Read.All の権限を承諾するように表示された場合は、許可する必要があります。
