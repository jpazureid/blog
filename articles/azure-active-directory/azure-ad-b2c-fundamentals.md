...
もし、あなたが自社組織の ID 管理基盤に求める機能が必要な場合、Azure AD B2C は適切なソリューションではない可能性があります。つまり、求める機能は実現できない、あるいは自身で細かい実装が必要になる可能性があるので、ご注意ください。

#### Azure AD B2C 以外のソリューション

Azure AD B2C を利用する以外に、「既存の Azure AD テナントに B2B ユーザーとしてゲスト招待をし、組織内のアプリケーションにサインインをさせる」 (シングル テナント アプリ)、あるいは、「マルチテナント アプリケーションを作成し、すべての Azure AD テナントのユーザーがサインイン可能なアプリを作る」ことが代替策になる可能性があります。
...
- 外部 ID の比較 - Azure Active Directory | Microsoft Docs
...
現実世界のネットショップでは、生年月日、電話番号、住所などの属性も収集が必要でしょう。また Azure AD B2C が標準で対応している本人確認はメールアドレスの確認のみですが、必要に応じて外部サービスや、独自の API との API 連携で本人確認を実施できます。

サインアップが完了すると、Azure AD B2C テナント上にユーザーが作成され、サインアップ時に利用したアカウント情報 (ソーシャルアカウントの情報、あるいはメールアドレス情報) と属性値が Azure AD 上に記録されます。
...
コンシューマー ユーザーは、赤枠のフローを通してサインアップ・サインインを行いアプリに対し、トークンを発行することが可能です。
...
> ※ カスタム ポリシーは ID プロバイダーとユーザー フローをより高度なカスタマイズをし利用したい、ID プロフェッショナル向けの機能です。
> Azure AD B2C をご利用いただく際には、まずは ID プロバイダーとユーザー フローにて要件を満たせるか検討いただくことを推奨します。
...
なお、Azure AD B2C テナントにユーザー (上記例では admin@contosob2c.onmicrosoft.com など) を作成いただき、グローバル管理者権限を付与することで、管理ユーザーとしてご利用いただくことも可能です。その場合、直接 Azure AD B2C テナントにアクセスするためテナントの切り替えが不要なほか、[Graph Explorer](https://developer.microsoft.com/ja-jp/graph/graph-explorer) などで API 呼び出しを試していただくことも可能です。そのため、検証用に Azure AD B2C テナントのメンバー ユーザーである管理ユーザーを作成いただくことをお勧めします。
...
テナントの管理ユーザー権限、あるいはサービス プリンシパル権限で Microsoft Graph API を呼び出すには、テナントにシングルテナント (またはマルチテナント) アプリを登録します。
...
### Azure AD B2C テナントに Azure AD Connect を利用しオンプレミス AD のユーザーを同期したいのですが可能ですか？
