...
> [!TIPS]
> Security Copilot を利用される際は請求される金額にご注意ください。Security Copilot は質問を処理する SCU (Security Conpmuter Unit）の稼働時間に応じて課金が生じます。Security Copilot を構成した Azure サブスクリプションに対して金額のアラートを設定しておき予算で設定した閾値に達したら Email で通知するか、Security Copilot 利用しないときは SCU を削除するかなど、予想外の請求が生じないよう準備されることをお勧めします。
