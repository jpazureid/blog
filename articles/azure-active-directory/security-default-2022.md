---
title: "2022 年 6 月末から「セキュリティの既定値群」が自動で有効化されます (対象 : 一部のテナント)"
date: 2022-6-3
tags:
  - Azure AD
  - MFA
...
# 2022 年 6 月末から「セキュリティの既定値群」が自動で有効化されます (対象 : 一部のテナント)
...
なお英語記事の直訳ではございませんため、記事の構成などが大幅に異なる点、ご容赦くださいませ。

## この記事の要点は ?
- 変更点 : **2019 年 10 月 22 日より前に作成された一部の Azure AD テナントにおいて、「セキュリティの既定値群」が自動で有効化されます。実施時期の目安は 2022 年 6 月末頃を予定しています。**
  
- この変更による影響を受けないことが期待されるテナント
  - 2019 年 10 月以降に作成された Azure AD テナント
  - 2019 年 10 月以前に作成された Azure AD テナントのうち、下記に該当するテナント
    - 現在「セキュリティの既定値群」を有効化している。
    - 現在「条件付きアクセス ポリシー」を利用している。
    - 以前に「セキュリティの既定値群」を構成したことがある。

- 影響を受ける可能性があるテナント
  - 上記のいずれにも該当しないテナント
  - 上記のいずれに該当しない場合であっても「セキュリティの既定値群」の自動有効化は段階的に実施される予定なので、即座には影響を受けない可能性もあります。レガシ認証クライアントの利用が比較的少ないテナントが、優先的に対象となる予定です。
...
## 本題 : 2019 年 10 月以前に作成された一部のテナントでは、自動でセキュリティの既定値群が有効化される予定です。
既に多くのお客様環境において「セキュリティの既定値群」や「条件付きアクセス ポリシー」を利用した MFA をご活用いただいております。
しかしいずれも無効化されているテナントの場合、セキュリティ観点では安全な状態とは言えません。このようなテナントを対象に、**2022 年の 6 月末を目安に、セキュリティの既定値群が自動で有効化される予定**です。

### セキュリティの既定値群の自動有効化の流れ
1. セキュリティの既定値群の自動有効化の対象テナントには、まず、グローバル管理者宛に、メールによる通知が送信される予定です。
...
4. 上記プロンプトで "Ask Later" を選択いただくと、**14 日以内にセキュリティの既定値群が自動で有効化されます。**
