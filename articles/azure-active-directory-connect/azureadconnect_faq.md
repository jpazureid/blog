...
Azure AD Connect (AADC) のお問い合わせが多いご質問について、以前 Q&A 形式でおまとめいたしておりましたが、バージョンも v2 となり記事が古くなってまいりました。
...
詳細は[こちら](https://learn.microsoft.com/ja-jp/azure/active-directory/hybrid/connect/how-to-connect-sync-staging-server#disaster-recovery) をご覧ください。

何か問題が発生した際には、ステージング サーバーとモードを切り替えて対応します。
この際、先に問題が起きているサーバーをステージングとし、もともとステージングだったサーバーは後からアクティブにしてください。
これは、短い時間ではありますが、2台のサーバーが同時にアクティブな状態となってしまうことを防ぐためです。
...
**A.** バックアップ/リストアによって Azure AD と オンプレミスAD 間で情報の不整合が生じる可能性があるためです。
...
しかしながら、バックアップからリストアされたAzure AD Connect はその変更を検知できないため、5/5 に作成・同期された 3 名は、 Azure AD にユーザー アカウントが残留してしまいます。
...
具体的には、2023 年 5 月 17 日現在、最新の Azure AD Connect のバージョンは、2022 年 11 月 9 日にリリースされたバージョン2.1.20.0 となっております。
...
なお、問題の発生を防ぐため、同期サイクルを変更した場合も、少なくとも 7 日に 1 度は同期が行われるようスケジュールを構成ください。
[Azure AD Connect 同期: Scheduler - Microsoft Entra | Microsoft Learn](https://learn.microsoft.com/ja-jp/azure/active-directory/hybrid/connect/how-to-connect-sync-feature-scheduler)
...
SQL の既定の動作では、SQL Express サーバー含め、そのサーバーで使用可能なメモリはすべて SQLが利用できるように構成されております。
...
SQL Server Management Studio から設定が可能ですので、下記公開情報をご確認ください。
...
Q. 完全同期は定期的に行うべきですか？
A. はい。オンプレミス側でのオブジェクトの追加・変更の頻度にもよりますが、可能であれば週に1度、月に1度といった定期的な頻度で完全同期を行うことを推奨しています。
これは、オンプレミス側で多くのオブジェクトを変更するなど、大量の変更が発生した場合に、差分同期では正常に同期されないオブジェクトが発生する場合がごく稀にあるためです。
過去事例でも、差分同期では同期されないオブジェクトが出てしまった旨が報告されています。
このシナリオでは、完全同期を行うことで整合性を取ることができ、事象が改善するため、定期的に完全同期を実施することをお勧めしています。
...
Q. ソースアンカーを objectGUID から mS-DS-ConsistencyGUID に変更した場合の影響について
A. 特に影響やリスクはありません。詳細は下記ブログも併せてご確認ください。

ハードマッチによる同期ユーザーの切り替え方法 (AD フォレスト移行 編) | Japan Azure Identity Support Blog (jpazureid.github.io)
...
A. ハード マッチを利用して、同期元のユーザーを切り替えます。詳細は下記ブログにて案内しておりますのでご確認ください。
ハードマッチによる同期ユーザーの切り替え方法 (AD フォレスト移行 編) | Japan Azure Identity Support Blog (jpazureid.github.io)
...
A. 
Azure AD Connect には、大量のオブジェクトがオンプレミス Active Directory で削除された場合に、閾値 (既定で 500) を超えた場合には削除処理が誤って行われた可能性を考慮し、同期を停止する機能が既定で有効に設定されています。削除処理が意図したものである場合には、一旦この保護機能を無効にして、同期を許可するという対応を実施します。
...
* コマンドを実行した後、"DeletionThreshold" にパラメーターを入力する必要がありますが、特に理由が無ければ既定値の 500 を指定します。

なお、閾値を変更することも Enable-ADSyncExportDeletionThreshold コマンドで -DeletionThreshold オプションを付与することで可能です。このとき特に一旦無効にする必要はありません。例えば 2 を設定すると 2 オブジェクト以上の削除が検出されると同期が止まります。

また、現在の閾値は Get-ADSyncExportDeletionThreshold で確認できます。

Q. インストール ウィザードが完了できません。
A. jpazureid/aadconnect-diagnostic: Azure AD Connect 資料採取ツール (github.com) のページを確認し、[シナリオトレース (構成ウィザード、またはその他のシナリオ)] を実施した際のログをお寄せください。


Q. permission-issue が発生しました。対処策を教えてください。
A. 同期元オブジェクトへの書き込みアクセス許可が不足している場合に発生します。
permission-issue - Error Code ; 8344 | Japan Azure Identity Support Blog (jpazureid.github.io) のブログをご確認ください。エラーの詳細をおまとめしています。
