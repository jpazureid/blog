...
これらのコマンドを利用してユーザー操作を必要とせずに定期的に情報を取得したり、リソースを変更するようなバッチ処理を行うようなアプリをワークロード ID と呼んでいます。
...
上記に対して、ワークロード ID の条件付きアクセスでは信頼できるネットワーク外からの認証をブロックすることで、サービス プリンシパルの不正使用を防ぐことができます。
...
動作としては思った通りの結果を得て、構成できる制御も少ないため目立った機能ではないかもしれないですが、個人的には遂にアプリケーションのサインインに対して制御を構成できるようになったことにとっても感心していました。Azure AD には様々な機能が追加されているため、今後の展開が楽しみです。
