...
条件付きアクセス ポリシーの「準拠済み」の設定は、デバイス ベースのアクセス制御となるため、"どの端末からのアクセスか？" を Microsoft Entra ID が判断する必要があります。この判断のために、ご利用のデバイスは Microsoft Entra ID に対して "デバイス情報" を提示する必要があります。
