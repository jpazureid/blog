...
「アプリの登録」の画面では、開発したアプリに対する API のアクセス許可を設定したり、アプリが Azure AD へ接続する際に利用する証明書やシークレットの管理が行えます。
...
アプリケーション オブジェクトは、サービスプリンシパル オブジェクトのテンプレートのようなもので、このテンプレートを基に各テナント内にサービスプリンシパル オブジェクトが作成されます。
各テナントでは、作成されたサービスプリンシパル オブジェクトに対して、ユーザーの割り当てなどを行いアクセス許可を定義することができます。
