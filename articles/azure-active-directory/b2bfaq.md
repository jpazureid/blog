...
たとえば、 chkoide@microsoft.com と Chihiro.Koide@microsoft.com のアドレスは、一見異なるアドレスですが、この 2 つのアドレスはどちらも同じユーザー アカウントに紐づいています。どちらのアドレスにメールを送信しても同じユーザーに届きます。

このような状態で招待を行った場合、あとから招待されたアドレスでは、ゲスト ユーザー オブジェクトは作成されません。

chkoide@microsoft.com のゲストがいる状態で、最新の Azure ポータルの画面から Chihiro.Koide@microsoft.com のアドレスを招待すると、以下のような動作となります。（2023/4/28 現在）
- 「メール」属性の値が Chihiro.Koide@microsoft.com に代わります。
- 「その他のメール」属性に、Chihiro.Koide@microsoft.com が追加されます。
- 「プロキシ アドレス」属性のプライマリ（SMTP）に Chihiro.Koide@microsoft.com が追加され、既存の chkoide@microsoft.com はセカンダリ（smtp）になります。
