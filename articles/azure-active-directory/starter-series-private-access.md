...
Global Secure Access (GSA) は、この**ゼロ トラスト**に基づいて設計している、Microsoft Entra の管理センターに統合されたネットワーク セキュリティ ソリューションです。「誰が・どのデバイスから・どのような状態でアクセスしているか」という ID 情報をもとに、クラウド上でアクセス制御を行います。
...
1. ユーザーのクライアント PC やモバイル端末 ( Windows 端末の場合 Microsoft Entra joined / Microsoft Entra hybrid joined / Microsoft Entra registered、Windows 端末以外の場合 Microsoft Entra registered ) に **GSA クライアント**がインストールされています
...
- **特徴**: 1 つのエンタープライズ アプリとして管理され、すべてのリソースに対して同じアクセス ポリシーが適用
...
1. クライアント端末に GSA クライアントをインストールします。インストール手順の詳細は以下の公開情報をご参照ください。   
   - GSA クライアントのダウンロード先は[こちら](https://entra.microsoft.com/#view/Microsoft_Azure_Network_Access/Clients.ReactView)です
...
> 参考: [チュートリアル: クイック アクセスによる VPN の置き換え | Microsoft Learn](https://learn.microsoft.com/ja-jp/entra/global-secure-access/tutorial-private-access-vpn-replacement)
...
1. タスク トレイの GSA アイコンを右クリック > **詳細な診断** を選択します
2. **転送プロファイル** タブ: 現在適用されているルールの一覧を確認します
3. **正常性チェック** タブ: クライアントの接続状態やプロファイルの最終チェック日時を確認します
